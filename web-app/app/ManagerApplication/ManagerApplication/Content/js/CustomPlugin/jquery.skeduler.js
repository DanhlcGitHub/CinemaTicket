    ...
    */
    ...
    function appendTasks(placeholder, tasks) {
        ...
        for (var i = 0; i < args.args.length; i++) {
            //var width = 294 / (args.args[i] || 1);
            // danhlc added if task.length!=0
            var width = 147;
            if (tasks.length > 0) {
                tasks[i].width = width;
                if (tasks[i].startTime % 2 == 0) {
                    tasks[i].left = 4;
                } else {
                    tasks[i].left = 150;
                }
        ...
        tasks.forEach(function (task, index) {
            ...
            var width = task.width || 294;
            var left = task.left || 4;
            //danhlc added
            var display = task.display;
            var title = task.title;
            var backgroundColor = task.backgroundColor;

            var card = $('<div></div>')
              .attr({
                  style: 'top: ' + top + 'px; height: ' + (height - 4) + 'px; ' + 'width: ' + (width - 8) + 'px; left: ' + left + 'px;' + 'display: ' + display + "; background-color: "
                      + backgroundColor + "; ",
                  title: title
              });
